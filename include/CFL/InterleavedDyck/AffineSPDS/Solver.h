...
public:
  ...
  bool mayReach() const { return verdict_ == Verdict::MayReach; }
  ...
  const std::optional<SeparationCertificate> &certificate() const { return certificate_; }
...
private:
  ...
  Verdict verdict_;
  std::optional<SeparationCertificate> certificate_;
};
...
struct Options {
  ...
  spds::Limits limits; // per projection; failures throw, never return partial bounds
...
struct Result {
  PairSet upper_bound;
  PairSet spds_upper_bound;
  PairSet independent_upper_bound;
  Statistics statistics;
  bool mayReach(Vertex source, Vertex target) const { return upper_bound.count({source,target}) != 0; }
};
...
public:
  ...
  bool spdsMayReach(Vertex vertex) const { return compare(vertex).spdsMayReach(); }
  bool independentMayReach(Vertex vertex) const { return compare(vertex).independentMayReach(observer()); }
  bool parenthesisReachable(Vertex vertex) const { return !compare(vertex).callHistory().empty(); }
  bool bracketReachable(Vertex vertex) const { return !compare(vertex).fieldHistory().empty(); }
  ...
  HistoryComparison compareStacks(Vertex vertex, const std::vector<unsigned> &calls,
...
private:
  ...
  Options options_;
  std::map<Vertex, spds::State> controls_;
...
public:
  ...
  QueryResult analyzeFrom(const Graph &graph, Vertex source, const HistoryObserver &observer) const;
  QueryResult analyzeTo(const Graph &graph, Vertex target) const;
  QueryResult analyzeTo(const Graph &graph, Vertex target, const HistoryObserver &observer) const;
  Result analyze(const Graph &graph) const;
  Result analyze(const Graph &graph, const HistoryObserver &observer) const;
private:
  QueryResult query(const Graph &graph, Vertex anchor, spds::Direction direction,
