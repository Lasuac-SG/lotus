...
public:
  ...
  Weight weight(const SynchronizedConfiguration &query) const {
    ...
    if (node == field_controls_.end())
      return call_.domain().zero();
    std::vector<Symbol> fields, calls;
    if (!encodeFields(query.fields, fields) || !encodeCalls(query, calls) ||
        !field_.accepts(node->second, fields))
      return call_.domain().zero();
...
private:
  ...
  SynchronizedResult(Automaton<Domain> call, Automaton<BooleanSemiring> field,
      ...
      : call_(std::move(call)), field_(std::move(field)),
        call_controls_(std::move(call_controls)),
        field_controls_(std::move(field_controls)),
        statements_(std::move(statements)), fields_(std::move(fields)) {}
  bool encodeFields(const std::vector<Field> &input,
  ...
  bool encodeCalls(const SynchronizedConfiguration &c,
                   std::vector<Symbol> &out) const {
    auto top = statements_.find(c.node.statement);
    if (top == statements_.end())
      return false;
...
public:
  using Weight = typename Domain::Weight;
  explicit SynchronizedSystem(Domain domain = Domain())
      : domain_(std::move(domain)) {}
  void addNode(FlowNode node) { nodes_.insert(node); }
  void addNormal(FlowNode from, FlowNode to) {
    addNormal(from, to, domain_.one());
  }
  ...
  void addReturn(FlowNode exit, FlowNode to) {
    addReturn(exit, to, domain_.one());
  }
...
private:
  ...
  SynchronizedResult<Domain> analyze(const SynchronizedConfiguration &seed,
    ...
    for (const auto &t : transfers_) {
      ...
      if (t.kind == Kind::Load)
        heap.addRule(a, fields.at(t.field), b, {});
      else if (t.kind == Kind::Store)
        heap.addPushRule(a, b, fields.at(t.field));
      else
        heap.addPreserveRule(a, b);
    }
    std::vector<Symbol> call_stack{statements.at(seed.node.statement)},
        field_stack;
    for (Statement s : seed.context)
      call_stack.push_back(statements.at(s));
    for (Field f : seed.fields)
      field_stack.push_back(fields.at(f));
    field_stack.push_back(0);
    auto call_seed = RegularSet::singleton(
        calls.controls(), {variables.at(seed.node.variable), call_stack});
    auto field_seed = RegularSet::singleton(
        heap.controls(), {locations.at(seed.node), field_stack});
    auto call_result = direction == Direction::Post
                           ? spds::postStar(calls, call_seed, limits)
                           : spds::preStar(calls, call_seed, limits);
    auto field_result = direction == Direction::Post
                            ? spds::postStar(heap, field_seed, limits)
                            : spds::preStar(heap, field_seed, limits);
    return SynchronizedResult<Domain>(
        std::move(call_result), std::move(field_result), std::move(variables),
        std::move(locations), std::move(statements), std::move(fields));
  }
  Domain domain_;
