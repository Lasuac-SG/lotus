...
private:
  // Ensure every discovered procedure has an entry-fact slot (default allTop).
  void seedEntries(const typename builder_t::Result &Built,
    ...
    for (const auto &Comp : Built.callGraph.order) {
      for (const auto &P : Comp.procs) {
        EntryFact.emplace(P, Problem.allTop());
      }
    }
    (void)InitialFact;
  }
      ...
      for (const auto &P : Comp.procs) {
        ...
        for (const auto &Inst : ICF.getAllInstructionsOf(P)) {
          ...
          for (const auto &Callee : ICF.getCalleesOfCallAt(Inst)) {
            ...
            const fact_t Merged =
                Problem.merge(CalleeIt->second, Contribution);
            if (!Problem.equal_to(Merged, CalleeIt->second)) {
  ...
  void emit(const typename builder_t::Result &Built, interp_t &Interp,
    ...
    for (const auto &Comp : Built.callGraph.order) {
      for (const auto &P : Comp.procs) {
        ...
        const fact_t In = EF == EntryFact.end() ? Problem.allTop() : EF->second;
