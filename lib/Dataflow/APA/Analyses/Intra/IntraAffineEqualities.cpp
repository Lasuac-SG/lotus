...
public:
  ...
  AffineFact meet(const AffineFact &Lhs,
                  const AffineFact &Rhs) const override {
    return D::combine(Lhs, Rhs);
  }

  bool equal_to(const AffineFact &Lhs, const AffineFact &Rhs) const override {
    return D::equal(Lhs, Rhs);
  }

  // Neutral element of the join (bottom).
  AffineFact meetIdentity() const override { return D::zero(); }
...
AffineResult runIntraElimAffine(llvm::Function *F, EliminationOptions Opts) {
  ...
  IntraEliminationSolver<LLVMEliminationDomain<AffineFact>> Solver(Problem,
                                                                   Opts);
